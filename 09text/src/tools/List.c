...
List* sj_newList(void){
	...
	if(this == NULL) sj_handleException(true, false, "Failed to malloc List.", NULL);
...
static void add(List* l, void* item){
	...
	if(this == NULL) sj_handleException(true, false, "Failed to malloc ListNode.", NULL);
...
static void* get(List* l, int index){
	...
	if(dx >= this->count){
		dx = this->count -1;
		sj_handleException(false, true, "Some List has attempted to read unallocated memory. List has returned last valid entry.", NULL);	
	}
